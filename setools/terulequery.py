...
class TERuleQuery(mixins.MatchObjClass, mixins.MatchPermission, query.PolicyQuery):
    ...
    def results(self):
        ...
        if self.target_self and (self.target_regex or (self.target_indirect == False)):
            self.log.warning("Ignoring specification of target regex and target indirect when used with --self")
